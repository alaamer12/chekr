...
export function pairsToViolations(pairs) {
	...
	for (const { code, primary, duplicate } of pairs) {
		...
		const message = buildMessage(code, primary, duplicate, sharedPathHint);
		...
		violations.push({
			...
			message: `${message} ${ignoreSuffix(code)}`,
			fix,
			code,
			occurrences: [
				{
					...
					line: primary.line,
					text: `${primary.kind} ${primary.name}`,
					context: "Primary declaration"
				}
			]
		});
	}
