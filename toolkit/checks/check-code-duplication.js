...
import { compareBlocks, buildTree, serializeBlock, DEFAULT_CONFIG } from "../utils/jsx-dedup.js";
...
export function pairsToViolations(pairs) {
	return pairs.map(({ score, a, b }) => {
		...
		return {
			...
			text: b.name,
			message:
				`Duplication score ${score} (>${DUPLICATION_SCORE_THRESHOLD}): ` +
				`${a.name} in ${a.file} (${rangeA}) ≈ ${b.name} in ${b.file} (${rangeB}). ` +
				`Extract shared JSX into ${hint}.`,
...
export async function checkCodeDuplicationRepo(scanPath = ".", prefilteredFiles = null, onProgress = null, context = null) {
	...
	for (const group of bySignature.values()) {
		...
		for (let i = 0; i < group.length; i++) {
			for (let j = i + 1; j < group.length; j++) {
				...
				// MESH OPTIMIZATION: Skip comparison if both files are unmodified
				if (context?.optimize && context.unmodifiedFiles?.has(a.file) && context.unmodifiedFiles?.has(b.file)) continue;
	...
	// Count pairs that were skipped by mesh optimization
	...
	for (const group of byScope.values()) {
		...
		for (let i = 0; i < group.length; i++) {
			for (let j = i + 1; j < group.length; j++) {
				...
				// MESH OPTIMIZATION: Skip comparison if both files are unmodified
				if (context?.optimize && context.unmodifiedFiles?.has(a.file) && context.unmodifiedFiles?.has(b.file)) {
					meshSkippedPairs++;
					continue;
				}
	...
	// MESH OPTIMIZATION: Restore old violations where both files were unmodified
	if (context?.optimize && context.cachedViolations?.length > 0) {
		const restored = context.cachedViolations.filter(v => {
			const fs = v._files;
			if (!fs || fs.length !== 2) return false;
			return context.unmodifiedFiles.has(fs[0]) && context.unmodifiedFiles.has(fs[1]);
		});
		return { violations: [...restored, ...newViolations], meshSkippedPairs };
	}

	return { violations: newViolations, meshSkippedPairs };
}
...
if (isMainEntryPoint(import.meta.url)) {
	...
	const violations = checkCodeDuplicationRepo(scanPath);

	if (violations.length === 0) {
		console.log(pass("✅ No code duplication violations found"));
		process.exit(0);
	}

	console.log(fail(`❌ ${violations.length} code duplication violations found\n`));
	printViolations(violations);

	if (reportPath) {
		writeReport(reportPath, {
			steps: [{ step: 41, name: "check-code-duplication", status: "fail", violations }],
