...
**CLI**
- [ ] `chekr run` — full scan
- [ ] `chekr fix` — dry-run fixers
- [ ] `chekr fix --apply` — apply fixers
- [ ] `chekr init` — scaffold example files
- [ ] `chekr validate` — validate rule files
- [ ] `--reporter default | json | compact`
- [ ] `--report <file>` — write report to file

**Config**
- [ ] `chekr.config.js` support
...
- [ ] Cache stored in `.chekr-cache/`
- [ ] `--no-cache` flag to bypass
- [ ] Cache invalidation on rule source change
- [ ] Cache invalidation on chekr version change
...
- [ ] `--watch` flag alias for `chekr run --watch`
...
- [ ] Ignored blocks report — lists every `@chekr-ignore` block with justification
...
- [ ] SQLite database at `.chekr/safe.db` — add `better-sqlite3` dependency to `@chekr/core`
- [ ] `SafeDB` module — `open()`, `save()`, `filter()`, `reset()` in `@chekr/core/safe-db.js`
...
- [ ] `chekr run --save-safe` — run checks then save all violations to the database
- [ ] `chekr violations.json --save-safe` — import violations from a JSON report file
- [ ] `chekr --reset` — delete safe.db with confirmation prompt
- [ ] `chekr --reset --yes` — skip confirmation (for scripts)
...
- [ ] Document `.chekr/safe.db` gitignore/commit decision in README
...
- [ ] `rules` map in `chekr.config.js` — override per-check severity without editing rule files
- [ ] Config severity takes precedence over check file's own export
- [ ] `--level <info|warning|error|1|2|3>` flag on `chekr run`
...
- [ ] `chekr list` shows severity column
- [ ] `chekr validate` warns if `severity` export is not `info | warning | error | 1 | 2 | 3`
...
- [ ] Rule packages — `npm install @chekr/rules-react` adds rules to your project
...
// chekr.config.js
export default {
  extends: ['@chekr/rules-react', '@chekr/rules-tokens'],
  rules: {
...
- [ ] `chekr run --format ai` — output violations in a format optimized for AI context
...
- Rule testing utilities — `@chekr/testing` with `expectViolation()` and `expectNoViolation()`
